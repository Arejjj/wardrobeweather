...
export default function OutfitSuggestion({ outfit, weather, onReshuffle }) {
  if (!outfit || outfit.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 shadow-sm">
        <Shirt className="mx-auto text-gray-300 mb-3" size={40} />
        <p className="text-gray-400">Keine passenden Kleidungsstücke gefunden.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
        <div>
          <h2 className="font-semibold text-gray-800 text-lg">Outfit für heute</h2>
          <p className="text-sm text-gray-400">Basierend auf {weather?.temp}° und {weather?.condition}</p>
        </div>
        <button
          onClick={onReshuffle}
        ...
        {outfit.map(item => (
          <div key={item.id} className="flex items-center gap-4 px-6 py-4">
            ...
            <div className="flex gap-1.5 flex-wrap justify-end">
              ...
              {item.rain && (
                <span className="text-xs bg-blue-50 text-blue-500 px-2 py-0.5 rounded-full">🌧️ Regen</span>
              )}
            </div>
          </div>
        ))}
